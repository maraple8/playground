package com.xfhuang.playground.user.biz.service.impl;
...
import com.xfhuang.framework.biz.context.holder.LoginUserContextHolder;
import com.xfhuang.framework.common.enums.DeletedEnum;
import com.xfhuang.framework.common.enums.StatusEnum;
import com.xfhuang.framework.common.exception.BizException;
import com.xfhuang.framework.common.response.Response;
import com.xfhuang.framework.common.util.DateUtils;
import com.xfhuang.framework.common.util.JsonUtils;
import com.xfhuang.framework.common.util.NumberUtils;
import com.xfhuang.framework.common.util.ParamUtils;
import com.xfhuang.playground.count.dto.FindUserCountsByIdRspDTO;
import com.xfhuang.playground.user.biz.constant.MQConstants;
import com.xfhuang.playground.user.biz.constant.RedisKeyConstants;
import com.xfhuang.playground.user.biz.constant.RoleConstants;
...
import com.xfhuang.playground.user.biz.domain.dataobject.UserRoleDO;
import com.xfhuang.playground.user.biz.domain.mapper.RoleDOMapper;
import com.xfhuang.playground.user.biz.domain.mapper.UserDOMapper;
import com.xfhuang.playground.user.biz.domain.mapper.UserRoleDOMapper;
import com.xfhuang.playground.user.biz.enums.ResponseCodeEnum;
import com.xfhuang.playground.user.biz.enums.SexEnum;
...
import com.xfhuang.playground.user.biz.rpc.CountRpcService;
import com.xfhuang.playground.user.biz.rpc.DistributedIdGeneratorRpcService;
import com.xfhuang.playground.user.biz.rpc.OssRpcService;
import com.xfhuang.playground.user.biz.service.UserService;
...
public class UserServiceImpl implements UserService {
    ...
    public Response<?> updateUserInfo(UpdateUserInfoReqVO updateUserInfoReqVO) {
        ...
        // 玩乐圈号
        String playgroundId = updateUserInfoReqVO.getplaygroundId();
        if (StringUtils.isNotBlank(playgroundId)) {
            Preconditions.checkArgument(ParamUtils.checkplaygroundId(playgroundId), ResponseCodeEnum.playground_ID_VALID_FAIL.getErrorMessage());
            userDO.setplaygroundId(playgroundId);
            needUpdate = true;
        }
    ...
    public Response<Long> register(RegisterUserReqDTO registerUserReqDTO) {
        ...
        // 否则注册新用户
        // RPC: 调用分布式 ID 生成服务生成玩乐圈 ID
        String playgroundId = distributedIdGeneratorRpcService.getplaygroundId();
        ...
        UserDO userDO = UserDO.builder()
                .id(userId)
                .phone(phone)
                .playgroundId(playgroundId) // 自动生成小红书号 ID
                .nickname("小红薯" + playgroundId) // 自动生成昵称, 如：小红薯10000
    ...
    public Response<FindUserProfileRspVO> findUserProfile(FindUserProfileReqVO findUserProfileReqVO) {
        ...
        FindUserProfileRspVO findUserProfileRspVO = FindUserProfileRspVO.builder()
                ...
                .avatar(userDO.getAvatar())
                .nickname(userDO.getNickname())
                .playgroundId(userDO.getplaygroundId())
                .sex(userDO.getSex())
                .introduction(userDO.getIntroduction())
                .build();
